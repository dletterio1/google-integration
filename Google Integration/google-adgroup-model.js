...
/**
 * Google Ad Groups are required containers between Campaigns and Ads
 * They control targeting, bidding, and ad rotation at a granular level
 */
const GoogleAdGroupSchema = new Schema({
  // Core References (Following Sonik patterns)
  _organization: {
  ...
  type: {
    type: String,
    enum: [
      'DISPLAY_STANDARD',           // Display Network ads
      'DISPLAY_SMART_CAMPAIGN',     // Smart Display campaigns
      'VIDEO_TRUE_VIEW_IN_STREAM',  // Skippable in-stream ads
      'VIDEO_TRUE_VIEW_IN_DISPLAY', // Video discovery ads
      'VIDEO_BUMPER',              // 6-second bumper ads
      'VIDEO_NON_SKIPPABLE'        // Non-skippable in-stream ads
    ],
    required: true
  },
  
  // Bidding Configuration (micros = amount * 1,000,000)
  bidding: {
    cpc_bid_micros: Number,         // Cost per click
    cpm_bid_micros: Number,         // Cost per thousand impressions
    cpv_bid_micros: Number,         // Cost per view (YouTube)
    target_cpa_micros: Number,      // Target cost per acquisition
    bid_modifier: {
      type: Number,
      default: 1.0,
      min: 0.1,
  ...
  targeting: {
    ...
    audiences: [{
      ...
      audience_type: {
        type: String,
        enum: ['USER_LIST', 'USER_INTEREST', 'CUSTOM_AFFINITY', 'CUSTOM_INTENT', 'SIMILAR']
      },
      bid_modifier: {
        type: Number,
        default: 1.0
    ...
    demographics: {
      age_ranges: [{
        type: String,
        enum: ['AGE_RANGE_18_24', 'AGE_RANGE_25_34', 'AGE_RANGE_35_44', 
      ...
      parental_status: [{
        type: String,
        enum: ['PARENT', 'NOT_A_PARENT', 'UNDETERMINED']
      }],
      household_income: [{
        type: String,
        enum: ['INCOME_RANGE_0_50', 'INCOME_RANGE_50_75', 'INCOME_RANGE_75_100',
               'INCOME_RANGE_100_150', 'INCOME_RANGE_150_UP', 'INCOME_RANGE_UNDETERMINED']
      }]
    },
    
    // Content targeting (Display Network)
    content: {
      keywords: [{
        text: {
          type: String,
      ...
      topics: [String],              // Google Topic IDs
      placements: [{
        url: String,                 // Specific websites/apps
        type: {
          type: String,
          enum: ['WEBSITE', 'MOBILE_APP', 'VIDEO', 'YOUTUBE_CHANNEL']
        }
      }]
    },
  ...
  ad_rotation: {
    type: String,
    enum: ['OPTIMIZE', 'ROTATE_INDEFINITELY'],
    default: 'OPTIMIZE'
  },
...
GoogleAdGroupSchema.index({ _organization: 1, status: 1 });
GoogleAdGroupSchema.index({ _campaign: 1, status: 1 });
GoogleAdGroupSchema.index({ googleCustomerId: 1, googleCampaignId: 1 });
GoogleAdGroupSchema.index({ 'metrics.last_sync_at': 1 });

// Virtual for cost in currency units (from micros)
GoogleAdGroupSchema.virtual('metrics.cost').get(function() {
  return this.metrics.cost_micros ? this.metrics.cost_micros / 1000000 : 0;
});

// Virtual for ROI calculation
GoogleAdGroupSchema.virtual('metrics.roi').get(function() {
  if (this.metrics.cost_micros === 0) return 0;
  return ((this.metrics.conversion_value - (this.metrics.cost_micros / 1000000)) / 
          (this.metrics.cost_micros / 1000000)) * 100;
});

// Method to update metrics from Google Ads API
GoogleAdGroupSchema.methods.updateMetrics = function(metricsData) {
  // Update raw metrics
  this.metrics.impressions = metricsData.impressions || 0;
  this.metrics.clicks = metricsData.clicks || 0;
  this.metrics.cost_micros = metricsData.cost_micros || 0;
  this.metrics.conversions = metricsData.conversions || 0;
  this.metrics.conversion_value = metricsData.conversions_value || 0;
  
  // Calculate derived metrics
  if (this.metrics.impressions > 0) {
    this.metrics.ctr = (this.metrics.clicks / this.metrics.impressions) * 100;
    this.metrics.average_cpm = (this.metrics.cost_micros / this.metrics.impressions) * 1000;
  }
  
  if (this.metrics.clicks > 0) {
    this.metrics.average_cpc = this.metrics.cost_micros / this.metrics.clicks;
    this.metrics.conversion_rate = (this.metrics.conversions / this.metrics.clicks) * 100;
  }
  
  // Video metrics if applicable
  if (metricsData.video_views) {
    this.metrics.video_views = metricsData.video_views;
    this.metrics.video_quartile_25_rate = metricsData.video_quartile_p25_rate || 0;
    this.metrics.video_quartile_50_rate = metricsData.video_quartile_p50_rate || 0;
    this.metrics.video_quartile_75_rate = metricsData.video_quartile_p75_rate || 0;
    this.metrics.video_quartile_100_rate = metricsData.video_quartile_p100_rate || 0;
    
    if (this.metrics.video_views > 0) {
      this.metrics.average_cpv = this.metrics.cost_micros / this.metrics.video_views;
    }
  }
  
  this.metrics.last_sync_at = new Date();
  return this.save();
};

// Method to check if ad group can be activated
GoogleAdGroupSchema.methods.canActivate = function() {
  // Must have at least one ad
  if (this.ad_count === 0) {
    return { 
      canActivate: false, 
      reason: 'Ad group must have at least one ad before activation' 
    };
  }
  
  // Must have valid bidding
  const hasBidding = this.bidding.cpc_bid_micros || 
                     this.bidding.cpm_bid_micros || 
                     this.bidding.cpv_bid_micros ||
                     this.bidding.target_cpa_micros;
  
  if (!hasBidding) {
    return { 
      canActivate: false, 
      reason: 'Ad group must have a valid bid amount' 
    };
  }
...
// Static method to find ad groups by campaign
GoogleAdGroupSchema.statics.findByCampaign = async function(campaignId, organizationId) {
  return this.find({
...
GoogleAdGroupSchema.pre('save', async function(next) {
  // Ensure YouTube ad groups have video targeting
  if (this.type.startsWith('VIDEO_') && !this.isNew) {
    const hasVideoTargeting = (this.targeting.youtube.channels.length > 0 || 
                              this.targeting.youtube.videos.length > 0 ||
                              this.targeting.audiences.length > 0);
    
    if (!hasVideoTargeting && this.status === 'ENABLED') {
      return next(new Error('YouTube ad groups must have at least one targeting option'));
    }
  }
...
GoogleAdGroupSchema.post('save', async function(doc) {
  const AuditLog = mongoose.model('AuditLog');
  
  if (this.wasNew) {
    await AuditLog.create({
      _organization: doc._organization,
      _user: doc._created_by,
      action: 'google_ad_group_created',
      resource: 'GoogleAdGroup',
      resourceId: doc._id,
      metadata: {
        campaignId: doc._campaign,
        googleAdGroupId: doc.googleAdGroupId,
        type: doc.type
      }
    });
  } else if (this.modifiedPaths().includes('status')) {
    await AuditLog.create({
      _organization: doc._organization,
      _user: doc._updated_by,
      action: `google_ad_group_${doc.status.toLowerCase()}`,
      resource: 'GoogleAdGroup',
      resourceId: doc._id,
      metadata: {
        googleAdGroupId: doc.googleAdGroupId,
...
GoogleAdGroupSchema.pre('save', function(next) {
